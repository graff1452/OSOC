...
static void step_n_cycles(long long n)
{
    for (long long i = 0; i < n && !sim_finished && g_cycles < MAX_CYCLES && !difftest_mismatch && !g_wp_triggered; i++)
    {
        ...
        // Sample here, between the edges: pc/inst/dbg_mem_*/dbg_if_valid still
        // combinationally reflect the instruction actually executing THIS cycle.
        // Reading them after the next posedge (below) would instead show a preview
        // of the NEXT instruction's decode, since Verilator settles that same
        // combinational chain (including the next fetch) within the same eval()
        // that updates pc -- this is exactly the bug that caused false device-touch
        // detections before.
        //
        // Step 3 (multi-cycle IFU): the RTL now takes 2 cycles per instruction --
        // one FETCH cycle (dbg_if_valid=0), one EXEC cycle (dbg_if_valid=1). Only
        // the EXEC cycle is a real "retired instruction" from this harness's point
        // of view: during the FETCH cycle that follows, pc has already advanced to
        // the next fetch address but dbg_inst is still showing the JUST-EXECUTED
        // instruction's stale bits (ifu.v's r_inst hasn't updated yet) -- so
        // (pc, inst) at a FETCH-cycle sample point is a mismatched pair, not a new
        // instruction.
        //
        // Step 4 (multi-cycle LSU): dbg_if_valid is no longer sufficient on its
        // own -- a load/store's EXEC phase can now last 2 cycles (request +
        // wait-for-LSU), and dbg_if_valid stays high for BOTH of them. Sampling
        // retiring_now = dbg_if_valid here would make every consumer below fire
        // TWICE for a load/store, the exact bug this gating was built to prevent
        // in the first place. dbg_retire is a new RTL port that's 1 only on the
        // FINAL cycle of EXEC (1 cycle for everything, 2 for a load/store) --
        // sample that instead, at the same point pc/inst are already read, so
        // itrace/ftrace/DiffTest/watchpoints below act exactly once per real
        // instruction regardless of how many cycles it actually took.
        bool retiring_now = g_top->dbg_retire;
        g_touched_device = (g_top->dbg_mem_wen || g_top->dbg_mem_ren)
                          && is_device_addr(g_top->dbg_mem_addr);
        // mtrace: no change needed here -- dbg_mem_wen/dbg_mem_ren are already
        // gated inside minirv.v itself: if_valid (step 2, so they read 0 during a
        // FETCH-cycle sample) AND, as of step 4, also lsu_valid (so a stalled
        // load/store's 2-cycle EXEC only asserts them on the response cycle, not
        // the request cycle too). This block already only fires once per
        // instruction as a side effect of that RTL gating -- no C++ change needed.
        if (g_mtrace_enabled) {
        ...
        g_cycles++;
        if (g_ftrace_enabled && retiring_now) {
        ...
        // DiffTest: must only advance REF (ref_difftest_exec) once per real
        // instruction. Calling this on a FETCH-cycle sample too would silently
        // desync REF from DUT by executing an extra, nonexistent instruction on
        // NEMU's side -- a real correctness bug, not just a cosmetic double-log.
        if (difftest_enabled && !sim_finished && retiring_now) {
            difftest_check_one_instruction();
        }
        // Watchpoints: gated the same way, so an expression change is reported
        // once per instruction rather than being (re-)evaluated and potentially
        // re-triggered on the following FETCH cycle, where nothing has changed.
        if (!sim_finished && retiring_now) check_watchpoints();
